...
class Translate extends CActiveRecord
{
	...
	public function rules()
	{
		...
		return array(
			array('language, name_key, message', 'required'),
			array('language', 'length', 'max'=>5),
			array('name_key', 'length', 'max'=>100),
			// The following rule is used by search().
			// @todo Please remove those attributes that should not be searched.
			array('translate_id, language, name_key, message', 'safe', 'on'=>'search'),
		);
	}
	...
	public function search()
	{
		...
		$criteria->compare('name_key',$this->name_key,true);
